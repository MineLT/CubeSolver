...
labelFrameMelange = Label(frameMelange, text = "Melange",   bg = '#C2BCBA')
...
labelFrameTestMoteurs = Label(frameTestMoteurs, text = "Test Moteurs",   bg = '#C2BCBA')
...
labelFramePatron = Label(frameTitrePatron, text = "Patron du Rubik's cube", bg = '#C2BCBA')
...
labelFrameCamera = Label(frameTitreCamera, text = "Courbe des temps",   bg = '#C2BCBA')
labelFrameCamera.configure(font=fontConflabelFrameCamera)
labelFrameCamera.pack()

#===========================================================================================#
#Onglet control courbe
def Ouvrir_Fermer(etat):
    ...
    if open_closeCam == 1:
        ...
        ouvrir_fermer.config(text = "Afficher courbe")
    else:
...
reset = Button(frameControlPatron, text = 'Reset couleur', command = resetAction)
reset.pack()
#===========================================================================================#
#Onglet test moteurs
def TestMoteurs():
    ...
    if open_closeTest == 1:
        frameTestMoteurs.pack_forget()
        frameImageDeFond.pack()
        testMoteurs.config(text = 'Test moteurs')
    else:
        frameImage.pack_forget()
        frameMelange.pack_forget()
        frameParametre.pack_forget()
        ...
        frameImageDeFond.pack_forget()
        frameAffichageGauche.pack_forget()
        ouvrir_fermer.config(text = "Allumer les caméras")
        melange.config(text = 'Melange')
        open_closeCam = 0
        open_closeMelange = 0
        open_closeParametre = 0
        frameTestMoteurs.pack(side = TOP)
        frameAffichageGauche.pack()
        testMoteurs.config(text = 'Fermer test moteurs')
    open_closeTest = 1 - open_closeTest

open_closeTest = 0
testMoteurs = Button(frameCommandeGauche, text = 'Test moteurs', command = TestMoteurs)
...
def TestMoteurs(moteur, action):
    ...
    if conteurTest > 50:
        showwarning('Alerte', 'Limite du test atteinte')
        return
    if action == 0:
...
texte = ['Une rotation sens horaire(90°)', 'Une rotation sens anti-horaire(90°)', 'Une rotation complete(180°)']
moteurs = ['Moteur U', 'Moteur F', 'Moteur R', 'Moteur B', 'Moteur L', 'Moteur D']
Face = ['U', 'F', 'R', 'B', 'L', 'D']
...
Mvt = StringVar(frameInfoTest, 'Mouvements effectués: ')
...
resteTest = Button(frameInfoTest, text = 'Réinitialiser le test', command = ResetTest)
...
def TestMoteurs():
    ...
    if open_closeMelange == 1:
        ...
        frameAffichageGauche.pack()
        melange.config(text = 'Fermer melange')
    open_closeMelange = 1 - open_closeMelange

open_closeMelange = 0
melange = Button(frameCommandeGauche, text = 'Melange', command = TestMoteurs)
melange.pack(side = LEFT)

lebelEntry = Label(frameEntrezMelange, text = 'Entrez un mélange: ', bg = '#C5B2B2')
...
def viderEntry(e):
    if askyesno('Envoyer', 'Etes-vous sur de vouloir envoyer ce mélange sur le robot?'):
...
resolutionMelange = Button(frameEntrezMelange, text = 'Résolution', command = Resolution, bg = '#9F8787', activebackground = '#9F8787')
...
genererMelange = Button(frameEntrezMelange, text = 'Générer un Melange', command  = GenererMelange, bg = '#9F8787', activebackground = '#9F8787')
...
def ResetTimer(e):
    ...
    Resultat.insert('1.0', 'Rubik\'s cube résolu en: ' + str_time + '\n')
...
def ActiverModeOfficiel():
    ...
    str_time = "%02d:%02d:%02d" % (minute, seconde, centiemeSeconde)
    timer.itemconfigure(text_clock, text=str_time)
    modeOfficielBouton = Button(frameTitrePrinc, text = 'Désactiver le mode officiel', command = DesactiverModeOfficiel)
    modeOfficielBouton.pack(side = TOP, anchor = 'ne', padx = 5, pady = 5)

def DesactiverModeOfficiel():
    global modeOfficielBouton
    ouvrir_fermer.config(text = "Eteindre les caméras")
...
Label(frameTab, text = "Appuyer sur tab pour résoudre le rubik's cube", bg = '#C2BCBA').pack()
...
modeOfficiel = Button(frameCommandeGauche, text = 'Activer le mode Officiel', command = ActiverModeOfficiel)
...
def Parametre():
    ...
    if open_closeParametre == 1:
        frameParametre.pack_forget()
        frameImageDeFond.pack()
        ParametreBouton.config(text = "Paramètres du robot")
...
ParametreBouton = Button(frameCommandeGauche, text = 'Paramètres du robot', command = Parametre)
ParametreBouton.pack(side = LEFT)
open_closeParametre = 0

save = Button(frameParametre, text = "Sauvgarder", command = Save, bg = '#9F8787', activebackground = '#9F8787')
save.pack(side = TOP, anchor = "nw")

LabelVitesseDepartEntree = Label(frameParametre, text = "Vitesse de depart: ",  bg = '#C2BCBA')
...
LabelVitesseMaxEntree = Label(frameParametre, text = "Vitesse maximale: ",  bg = '#C2BCBA')
...
LabelAccelerationEntree = Label(frameParametre, text = "Accélération (en pas): ",  bg = '#C2BCBA')
...
reset = Button(frameImage, text = 'Reset le dernier', command = Reset_Last)
...
resetall = Button(frameImage, text = 'Tout reset', command = Reset_all)
